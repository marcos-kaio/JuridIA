...
export async function register({
  ...
  try {
    ...
    const message =
      err.response?.data?.error || 'Ocorreu um erro ao tentar registrar.';
    throw new Error(message);
  }
}

export async function updateEducation(escolaridade) {
  try {
    const resp = await api.put('/user/update-education', { escolaridade });
    return resp.data;
  } catch (err) {
    const message =
      err.response?.data?.error ||
      'Ocorreu um erro ao atualizar a escolaridade.';
    throw new Error(message);
  }
}
