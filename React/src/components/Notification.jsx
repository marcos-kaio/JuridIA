...
const Notification = () => {
  ...
  useEffect(() => {
    ...
    if (notification) {
      ...
      const timer = setTimeout(() => {
        setShow(false);
        // Espera a animação de saída terminar antes de limpar a notificação
  ...
  const baseClasses = "fixed top-5 right-5 flex items-center p-4 rounded-lg shadow-lg text-white transition-all duration-300 ease-in-out z-[2000]";
  const typeClasses = {
  ...
  return (
    <div className={`${baseClasses} ${typeClasses[type] || typeClasses.info} ${animationClasses}`}>
      {icons[type]}
      <span>{message}</span>
      <button onClick={() => setShow(false)} className="ml-4 p-1 rounded-full hover:bg-black/20">
